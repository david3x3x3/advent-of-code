            ...
            while True:
                ...
                for i in range(len(args)):
                    if flags % 10 == 0 and i != cmds[cmd][2]:
                        args[i] = mem[args[i]]
                    flags //= 10
                print(f'trace({amp_num}) {mem[pc]} {cmds[cmd][0]} {args}')
                if cmd == 1: # add
                    ...
                    if len(inp) == 0:
                        print(f'amp {amp_num} waiting for input')
                        a['pc'] = pc
                    ...
                    print(f'OUT = {args[0]} from {amp_num} to {(amp_num+1)%5}')
                    val = args[0]
                    amps[(amp_num+1)%5]['inp'] += [val]
                    if amp_num == 4:
                        output += [val]
                    pc += (len(args)+1)
                elif cmd == 5: # JMPT
